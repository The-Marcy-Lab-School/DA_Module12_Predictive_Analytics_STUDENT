...
**Fill this in on Day 1-2, before any real modeling work.** A charter
...
alongside the pipeline and memo, and it's what your individual
...
| Task | Project Manager | Technical Lead | Business Analyst | Experimentation Lead |
|---|---|---|---|---|
| Domain/target selection (team decision) | | | | |
| Train/test split + model fit (sections 3-4) | | | | |
| Evaluation metric justification (section 5) | | | | |
| Fairness/disparity check (section 6) | | | | |
| MLflow logging + retrieval (section 7) | | | | |
| Clustering/segmentation (section 8) | | | | |
| Business-framed interpretation (section 9) | | | | |
| A/B test design + real sample-size calc (section 10) | | | | |
| Second model comparison (section 11) | | | | |
| Second fairness segment (section 12) | | | | |
...
becomes a milestone-check-in issue.
...
2. **If it doesn't resolve, or someone's genuinely unresponsive for
   ...
   instructor directly — this is exactly the kind of thing instructors
   expect to help with mid-project, not a sign of failure to ask. Don't
   wait until the deadline to raise it for the first time.
3. **Document it factually in `individual_reflection_<name>.md`** if it
   affected your own work — real and specific, not accusatory.
